...
def mk_http_pcap(http_request, http_response, src_ip = "172.1.1.1", src_port = 5000, dst_ip = "172.1.1.2", dst_port = 80, pcapname='out'):
    ...
    # 将流量报文保存到本地为 .pcap 文件
    pathlib.Path('pcap').mkdir(parents=True, exist_ok=True) 
    wrpcap('pcap/'+pcapname+'.pcap', [http_traffic])
...
if __name__ == "__main__":
    ...
    req_body = 'xxxxxxxxxxxxxxxxxxxxxxxxxxx'
    reqfix = fix_content_length(http_response_prv+req_body)
    print(reqfix)
    mk_http_pcap(http_request=http_request_prv, http_response=http_response_prv+req_body, pcapname=r'')
